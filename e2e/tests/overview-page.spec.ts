...
test.describe('saved searches', () => {
  ...
  test('Clicking on a global saved search bookmark with custom order renders correctly', async ({
    ...
    // Get the 'search_id' query parameter from the url in the first request.
    const searchId = new URL(featuresRequests[0].url()).searchParams.get(
      'search_id',
    );
    expect(searchId).toBe('top-html-interop');
