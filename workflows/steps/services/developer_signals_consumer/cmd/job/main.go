...
import (
	...
	"github.com/GoogleChrome/webstatus.dev/workflows/steps/services/developer_signals_consumer/pkg/data"
...
func main() {
	...
	downloader, err := data.NewHTTPDownloader(http.DefaultClient)
	if err != nil {
		slog.ErrorContext(ctx, "failed to create downloader", "error", err.Error())
		os.Exit(1)
	}
	...
	processor := workflow.NewDeveloperSignalsProcessor(
		data.Parser{},
		downloader,
		spanneradapters.NewDeveloperSignalsConsumer(spannerClient))
