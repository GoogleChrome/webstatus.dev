...
export class WebstatusNotificationRssChannels extends LitElement {
  ...
  private async _handleSave(e: CustomEvent) {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        void toast(`Created RSS channel "${resp.name}".`, 'success');
      } else {
        // For update, we might only need to send name if that's what changed.
        // The dialog passes updates in e.detail.
        const resp = await this.apiClient.updateNotificationChannel(
          token,
          channelId,
          updates,
        );
        void toast(`Updated RSS channel "${resp.name}".`, 'success');
      }
      this.dispatchEvent(
      ...
      void toast(
        `Failed to ${mode} RSS channel. Please try again.`,
        'danger',
        'exclamation-triangle',
      );
      console.error(`Failed to ${mode} RSS channel`, e);
    } finally {
  ...
  private async _confirmDelete() {
    ...
    try {
      ...
      void toast(
        'Failed to delete RSS channel. Please try again.',
        'danger',
        'exclamation-triangle',
      );
      console.error('Failed to delete RSS channel', e);
    } finally {
  ...
  render() {
    return html`
      <webstatus-notification-panel>
        ...
        <div slot="content">
          ${this.channels.length === 0
            ? html`<p class="empty-message">No RSS channels configured.</p>`
            : repeat(
                this.channels,
                channel => channel.id,
                channel => html`
      ...
      <sl-dialog
        ...
        label="Delete RSS Channel"
        @sl-hide=${this._closeDeleteDialog}
      >
        <p>Are you sure you want to delete this RSS channel?</p>
        <sl-button
          slot="footer"
          variant="default"
