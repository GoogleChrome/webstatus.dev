...
export class FeaturePage extends BaseChartsPage {
  ...
  constructor() {
    ...
    this._startFeatureSupportTask();
    this._startFeatureUsageTask();
  ...
  private _startDataFetchingTask<
    T extends LoadingTaskType,
    F extends FetchTaskType,
  >(dataFetcher: F, taskType: T) {
    ...
    this[taskType] = new Task(this, {
      ...
      task: async ([apiClient, featureId, startDate, endDate]) => {
        await this[dataFetcher](apiClient, featureId, startDate, endDate);
      },
    });
  }

  private _startFeatureSupportTask() {
    this._startDataFetchingTask(
      '_fetchFeatureSupportData',
      '_loadingMetricsTask',
    );
  }

  private _startFeatureUsageTask() {
    this._startDataFetchingTask('_fetchFeatureUsageData', '_loadingUsageTask');
  }
}
