...
func validateNotificationChannel(input *backend.CreateNotificationChannelRequest) *fieldValidationErrors {
	...
	switch input.Type {
	case backend.CreateNotificationChannelRequestTypeEmail:
		_, err := input.Config.AsEmailConfig()
		if err != nil {
			fieldErrors.addFieldError("config", errors.New("invalid email config for type email"))
		}
	case backend.CreateNotificationChannelRequestTypeWebhook:
		cfg, err := input.Config.AsWebhookConfig()
		if err != nil {
			fieldErrors.addFieldError("config", errors.New("invalid webhook config for type webhook"))
		} else {
	...
	default:
		fieldErrors.addFieldError("type", errors.New("unsupported notification channel type"))
	}
