...
func validateNotificationChannel(input *backend.CreateNotificationChannelRequest) *fieldValidationErrors {
	...
	case backend.NotificationChannelResponseTypeEmail, backend.NotificationChannelResponseTypeRss:
		fieldErrors.addFieldError(
			"config",
			errors.New("invalid config: only webhook channels can be created manually"),
		)
	default:
		fieldErrors.addFieldError(
			"config",
