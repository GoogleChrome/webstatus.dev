...
func validateUpdateNotificationChannel(request *backend.UpdateNotificationChannelRequest) *fieldValidationErrors {
	...
	for _, mask := range request.UpdateMask {
		...
		case backend.UpdateNotificationChannelRequestMaskConfig:
			...
			// We don't know the config type here without fetching the existing channel,
			// but check if at least one is valid. The adapter layer will handle the channel type specific logic.
			_, errEmail := request.Config.AsEmailConfig()
			cfgWebhook, errWebhook := request.Config.AsWebhookConfig()
			if errEmail != nil && errWebhook != nil {
				fieldErrors.addFieldError("config", errors.New("invalid config: must be either email or webhook config"))
			}
			if errWebhook == nil {
				if err := validateSlackWebhookURL(cfgWebhook.Url); err != nil {
