...
func TestGenericStateAdapter_Load(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockConverter: func(_ *testSnapshot) (map[string]comparables.Feature, string, []workertypes.SummaryQueryError) {
				return nil, "", nil
			},
			wantSnapshot:  nil,
			wantID:        "",
		...
		},
		{
			...
			mockMigrator: func(b []byte) ([]byte, error) { return b, nil },
			mockConverter: func(s *testSnapshot) (map[string]comparables.Feature, string, []workertypes.SummaryQueryError) {
				if s.Data != "some-data" {
...
func TestV1DiffSerializer_Serialize(t *testing.T) {
	...
	diff := &featurelistdiffv1.FeatureDiff{
		SnapshotOrigin: featurelistdiffv1.OriginLive,
		QueryChanged:   false,
