	...
	tests := []struct {
		...
		},
		{
			...
			verifyMocks: func(t *testing.T, _ *mockStateAdapter,
				...
				if !workflow.setResolvedQueryErrorsCalled {
					t.Error("expected SetResolvedQueryErrors to be called")
				}
			},
		},
	}
...
func (m *mockSummaryGenerator) GenerateJSONSummary(_ v1.FeatureDiff) ([]byte, error) {
	return []byte(`{"resolvedQueryErrors":[{"code":"query_grammar_invalid"}]}`), nil
}
...
func TestRun_RealWorkflow_QueryErrorRecovery_NoSpam(t *testing.T) {
	...
	if len(serializedDiff.ResolvedQueryErrors) != 1 ||
		serializedDiff.ResolvedQueryErrors[0].Code != v1.ErrorCodeQueryGrammar {
		t.Errorf("expected 1 ResolvedQueryErrors with code QUERY_GRAMMAR_INVALID, got %v", serializedDiff.ResolvedQueryErrors)
	}
}
