...
func (c mockBackendSpannerClient) CreateNotificationChannel(
	...
	if !reflect.DeepEqual(req, c.mockCreateNotificationChannelCfg.expectedRequest) {
		c.t.Errorf("unexpected input to CreateNotificationChannel: got %+v, want %+v",
			req, c.mockCreateNotificationChannelCfg.expectedRequest)
	}
...
func (c mockBackendSpannerClient) UpdateNotificationChannel(_ context.Context,
	req gcpspanner.UpdateNotificationChannelRequest) error {
...
func TestCreateNotificationChannel(t *testing.T) {
	const (
		...
		expectedAPIType = backend.NotificationChannelTypeEmail
	)
	...
	}{
		{
			...
			createCfg: &mockCreateNotificationChannelConfig{
				expectedRequest: gcpspanner.CreateNotificationChannelRequest{
					UserID:      userID,
					Name:        channelName,
					Type:        string(expectedAPIType),
					EmailConfig: &gcpspanner.EmailConfig{Address: channelValue, IsVerified: false, VerificationToken: nil},
				},
			...
			expected: &backend.NotificationChannelResponse{
				Id:        channelID,
				Name:      channelName,
				Type:      backend.NotificationChannelResponseType(expectedAPIType),
				Value:     channelValue,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mock := mockBackendSpannerClient{
				t:                                t,
				mockCreateNotificationChannelCfg: tc.createCfg,
				mockGetNotificationChannelCfg:    tc.getCfg,
			}
			b := NewBackend(mock)
			resp, err := b.CreateNotificationChannel(context.Background(), userID, tc.input)
			if !errors.Is(err, tc.expectedError) {
...
func TestUpdateNotificationChannel(t *testing.T) {
	...
	testCases := []struct {
		name          string
		input         backend.UpdateNotificationChannelRequest
	...
	}{
		{
			...
			updateCfg: &mockUpdateNotificationChannelConfig{
				expectedRequest: gcpspanner.UpdateNotificationChannelRequest{
					ID:     channelID,
					UserID: userID,
					Name:   gcpspanner.OptionallySet[string]{Value: updatedName, IsSet: true},
			...
			getCfg: &mockGetNotificationChannelConfig{
				...
				result: &gcpspanner.NotificationChannel{
					...
					UserID:      "user",
					Type:        "email",
					EmailConfig: &gcpspanner.EmailConfig{Address: "old@example.com", IsVerified: false, VerificationToken: nil},
					CreatedAt:   now,
					UpdatedAt:   now,
				},
				returnedError: nil,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mock := mockBackendSpannerClient{
				t:                                t,
				mockUpdateNotificationChannelCfg: tc.updateCfg,
				mockGetNotificationChannelCfg:    tc.getCfg,
			}
			b := NewBackend(mock)
			resp, err := b.UpdateNotificationChannel(context.Background(), userID, channelID, tc.input)
			if !errors.Is(err, tc.expectedError) {
				t.Errorf("unexpected error. got %v, want %v", err, tc.expectedError)
			}
			if diff := cmp.Diff(tc.expected, resp); diff != "" {
