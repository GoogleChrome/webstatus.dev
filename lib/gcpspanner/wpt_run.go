...
// SpannerWPTRunFilterableInfo is a wrapper around WPTRunFilterableInfo. Useful
// for tests since ID is autogenerated.
type SpannerWPTRunFilterableInfo struct {
	ID string `spanner:"ID"`
	WPTRunFilterableInfo
}

// WPTRunFilterableInfo contains a subset of information that is used for the FeatureWPTRunMetricsLogs.
type WPTRunFilterableInfo struct {
	ExternalRunID int64     `spanner:"ExternalRunID"`
	TimeStart     time.Time `spanner:"TimeStart"`
	BrowserName   string    `spanner:"BrowserName"`
	Channel       string    `spanner:"Channel"`
}

// GetFilterableWPTRunInfoByExternalRunID is a helper function to help get the spanner ID of the
...
// It uses the RunsByExternalRunID index to quickly look up the row.
func (c *Client) GetFilterableWPTRunInfoByExternalRunID(
	ctx context.Context, externalRunID int64) (*SpannerWPTRunFilterableInfo, error) {
	txn := c.Single()
	defer txn.Close()

	stmt := spanner.NewStatement(`
SELECT
	ID, ExternalRunID, TimeStart, BrowserName, Channel
FROM WPTRuns
...
LIMIT 1
	...
	var ret SpannerWPTRunFilterableInfo
	err = row.ToStruct(&ret)
	if err != nil {
		return nil, errors.Join(ErrInternalQueryFailure, err)
	}
