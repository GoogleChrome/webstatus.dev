...
func TestNotificationChannelRefactoredOperations(t *testing.T) {
	...
	channelToUpdateID, err := spannerClient.CreateNotificationChannel(ctx, baseCreateReq)
	if err != nil {
		t.Fatalf("failed to pre-populate channel for update/delete tests: %v", err)
	}

	channelToDeleteID, err := spannerClient.CreateNotificationChannel(ctx, baseCreateReq)
	if err != nil {
