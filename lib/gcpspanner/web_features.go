...
func (m webFeatureSpannerMapper) GetChildDeleteKeyMutations(
	...
	defer txn.Close()
	for _, parent := range parentsToDelete {
		systemManagedSearch, err := client.getSystemManagedSavedSearchByFeatureIDAndTransaction(ctx, txn, parent.ID)
		if err != nil {
			if errors.Is(err, ErrQueryReturnedNoResults) {
				// No system-managed saved search, so nothing to do
				continue
			}
		...
		savedSearchMutations = append(savedSearchMutations,
			spanner.Delete(systemManagedSavedSearchesTable, spanner.Key{systemManagedSearch.FeatureID}))

		// Delete the saved search
		savedSearchMutations = append(savedSearchMutations,
			spanner.Delete(savedSearchesTable, spanner.Key{systemManagedSearch.SavedSearchID}))
	}
