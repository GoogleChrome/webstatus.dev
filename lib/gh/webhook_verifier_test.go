...
func TestVerifyWebhookSignature(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			payload:   payload,
			headerSig: "sha256= " + hex.EncodeToString(mac.Sum(nil)) + " ",
			secret:    validSecret,
			wantValid: true,
		},
		{
		...
		{
			name:      "too short secret (<16 bytes)",
			payload:   payload,
			headerSig: validSig,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := VerifyWebhookSignature(tc.payload, tc.headerSig, tc.secret)
			if got != tc.wantValid {
				t.Errorf("VerifyWebhookSignature() = %v, want %v", got, tc.wantValid)
			}
		})
	}
}
...
func FuzzVerifyWebhookSignature(f *testing.F) {
	...
	f.Fuzz(func(_ *testing.T, payload []byte, header string, secret []byte) {
		// Verify function never panics on arbitrary input
		_ = VerifyWebhookSignature(payload, header, secret)
	})
}
